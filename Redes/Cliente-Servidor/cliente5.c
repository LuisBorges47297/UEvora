...
void receive_file(int sockfd, const char *filename, int filesize) {
    ...
    fclose(file);
    printf("Arquivo %s (%d bytes) recebido com sucesso.\n", filename, filesize);
}
...
void send_file(int sockfd, const char *filename) {
    ...
    snprintf(upload_msg, BUFFER_SIZE, "UPLOAD %s %d\n", filename, filesize);
    send(sockfd, upload_msg, strlen(upload_msg), 0);

    // Wait for UPLOADREADY response
    char buffer[BUFFER_SIZE];
    ...
    buffer[n] = '\0';
    if (strncmp(buffer, "UPLOADREADY", 11) != 0) {
        fprintf(stderr, "ERROR: Server not ready for upload\n");
        fclose(file);
        return;
    }

    // Send the file in batches of 1024 bytes
    ...
    fclose(file);
    printf("Arquivo %s (%d bytes) enviado com sucesso.\n", filename, filesize);
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 3 || argc > 4) {
        fprintf(stderr,"Uso: %s <endereco> <porta> [<arquivo>]\n", argv[0]);
        exit(1);
    }
    ...
    while (1) {
        ...
        if (FD_ISSET(sockfd, &readfds)) {
            ...
            if (n > 0) {
                ...
                if (strncmp(buffer, "PING", 4) == 0) {
                    ...
                    // Processar mensagem FILE
                    char filename[BUFFER_SIZE];
                    int filesize;
                    sscanf(buffer + 5, "%s %d", filename, &filesize);
                    printf("Preparando-se para receber o arquivo: %s (%d bytes)\n", filename, filesize);

                    // Enviar resposta FILEREADY
                    send(sockfd, "FILEREADY\n", 10, 0);

                    // Receber o arquivo
                    receive_file(sockfd, filename, filesize);
                }
            } else if (n == 0) {
        ...
        // Check if there's an upload to be done (e.g., based on user input)
        if (argc == 4) {
            // Assuming the fourth argument is the filename to upload
            send_file(sockfd, argv[3]);
            // Reset argc to avoid continuous uploads
            argc = 3;
        }
    }
